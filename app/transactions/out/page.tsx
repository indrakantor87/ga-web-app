...
type OutListResponse = { rows?: Row[]; total?: number }
type ItemsResponse = {
...
export default function Page() {
  ...
  const [form, setForm] = useState({ id_barang: '', tanggal: fmtDateInput(today), jumlah: '', teknisi: '', keterangan: '' })
  const [editing, setEditing] = useState<Row | null>(null)

  const totalPages = useMemo(() => Math.max(1, Math.ceil(total / pageSize)), [total, pageSize])

  const refresh = async () => {
    const url = new URL('/api/transactions/out', window.location.origin)
    ...
    const obj = data as OutListResponse
  ...
  const save = async () => {
    ...
    try {
      ...
      const url = editing ? `/api/transactions/out/${editing.id}` : '/api/transactions/out'
  ...
  const onEdit = (row: Row) => {
    setEditing(row)
    setForm({
      id_barang: '', // akan diisi dari master items jika perlu, atau dibiarkan manual
      tanggal: row.tanggal,
  ...
  const onDelete = async (row: Row) => {
    ...
    const r = await fetch(`/api/transactions/out/${row.id}`, { method: 'DELETE' })
    const data = await r.json().catch(() => ({}))
    if (!r.ok) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Barang Keluar</h2>
          <p className="text-sm text-gray-500">Data barang keluar</p>
        </div>
      ...
      {modal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-xl rounded-2xl bg-white p-6 shadow-xl ring-1 ring-gray-200">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-bold text-gray-900">{editing ? 'Edit Barang Keluar' : 'Tambah Barang Keluar'}</h3>
            ...
            <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <div>
                <label className="text-sm font-semibold text-gray-700">Teknisi</label>
                <input value={form.teknisi} onChange={(e) => setForm((p) => ({ ...p, teknisi: e.target.value }))} className="mt-2 w-full rounded-lg border border-gray-200 px-3 py-2 text-sm" />
              </div>
              <div className="md:col-span-2">
