...
function Modal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto p-4">
      ...
      <div className="relative w-full max-w-lg max-h-[calc(100vh-2rem)] rounded-2xl bg-white shadow-xl ring-1 ring-gray-200 flex flex-col">
...
export default function MasterUnitsPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    setError(null)
    const url = new URL('/api/master/units', window.location.origin)
  ...
  const save = async () => {
    setSaving(true)
    try {
      const payload = { nama_satuan: namaSatuan.trim() }
      if (!payload.nama_satuan) throw new Error('Nama satuan wajib diisi')

      if (editing) {
        const r = await fetch(`/api/master/units/${editing.id_satuan}`, {
          method: 'PUT',
        ...
        const r = await fetch('/api/master/units', {
          method: 'POST',
          headers: { 'content-type': 'application/json' },
  ...
  const toggleActive = async (row: UnitRow) => {
    const next = row.is_active === 'ONE' ? 'ZERO' : 'ONE'
    const r = await fetch(`/api/master/units/${row.id_satuan}`, {
      method: 'PATCH',
      headers: { 'content-type': 'application/json' },
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Satuan</h2>
          <p className="text-sm text-gray-500">Kelola master satuan barang</p>
        </div>
        <button
          onClick={openAdd}
          ...
          <table className="min-w-full">
            <thead>
              <tr className="border-b border-gray-100 text-left text-xs font-semibold text-gray-500">
                ...
                <th className="py-3 pr-4">Nama Satuan</th>
            ...
            <tbody className="text-sm text-gray-700">
              {loading ? (
                ...
                rows.map((row, idx) => {
                  ...
                  return (
                    <tr key={row.id_satuan} className="border-b border-gray-50 last:border-0">
                      <td className="py-3 pr-4 text-gray-500">{(page - 1) * pageSize + idx + 1}</td>
                      <td className="py-3 pr-4 font-semibold text-gray-900">{row.nama_satuan}</td>
      ...
      <Modal open={modalOpen} title={editing ? 'Edit Satuan' : 'Tambah Satuan'} onClose={() => setModalOpen(false)}>
