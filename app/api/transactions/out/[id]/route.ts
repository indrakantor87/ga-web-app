...
export async function PUT(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
    ...
    if (old.id_barang === id_barang) {
      const delta = (old.jumlah ?? 0) - (jumlah ?? 0) // OUT: stok berkurang jumlah, jadi delta kebalikan
      if (delta !== 0) {
        await tx.tbl_barang.update({ where: { id_barang }, data: { stok: { increment: delta } } })
      }
    } else {
      // pindah barang: kembalikan stok lama, kurangi stok baru
      if (old.jumlah) {
        await tx.tbl_barang.update({ where: { id_barang: old.id_barang }, data: { stok: { increment: old.jumlah } } })
      }
      await tx.tbl_barang.update({ where: { id_barang }, data: { stok: { decrement: jumlah } } })
    }
